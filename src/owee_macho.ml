...
let reloc_type cpu_type n = match cpu_type, n with
  ...
  | `X86       , 03   -> `GENERIC_RELOC_PB_LA_PTR
  | `X86       , 04   -> `GENERIC_RELOC_LOCAL_SECTDIFF
  | `X86       , 05   -> `GENERIC_RELOC_TLV
  ...
  | `ARM, 00 -> `ARM_RELOC_VANILLA
  | `ARM, 01 -> `ARM_RELOC_PAIR
  | `ARM, 02 -> `ARM_RELOC_SECTDIFF
  | `ARM, 03 -> `ARM_RELOC_LOCAL_SECTDIFF
  | `ARM, 04 -> `ARM_RELOC_PB_LA_PTR
  | `ARM, 05 -> `ARM_RELOC_BR24
  | `ARM, 06 -> `ARM_THUMB_RELOC_BR22
  | `ARM, 07 -> `ARM_THUMB_32BIT_BRANCH
  | `ARM, 08 -> `ARM_RELOC_HALF
  | `ARM, 09 -> `ARM_RELOC_HALF_SECTDIFF
  ...
  | `ARM64, 11 -> `ARM64_RELOC_AUTHENTICATED_POINTER
  | _         , n     -> `Unknown n
